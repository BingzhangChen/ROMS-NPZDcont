...
#  elif defined  NPACIFIC 
      parameter (LLm0=760,  MMm0=304,  N=40)   ! 1/4 degree resolution
#  elif defined  NPAC 
      parameter (LLm0=378,  MMm0=150,  N=40)   ! 1/2 degree resolution
#  elif defined  NPACS 
      parameter (nameit='npacS')
      parameter (LLm0=120,  MMm0=60,  N=40) 
#  elif defined  NPAC_ATL
      parameter (LLm0=400,  MMm0=200,  N=40)   ! Northern Pacific and Atlantic
#  elif defined  GLOBAL
      parameter (nameit='Global')
      parameter (LLm0=1440, MMm0=560,  N=40)   ! Global Ocean 1/4 degree
#  else
...
!
      ...
      parameter (NP_XI=15, NP_ETA=1,  NNODES=NP_XI*NP_ETA)
      parameter (NPP=1)
...
! ====== == ======= === ====== ============== =======
!
#if defined BIO_NPZDDISC || defined BIO_EFTDISC
      integer, parameter :: NPHY = 20
      real               :: PMU_(NPHY)
      real,    parameter :: pi1=3.14159265358979323
      real,    parameter :: PMU_min =log(pi1/6d0*0.6**3)
      real,    parameter :: PMU_max =log(pi1/6d0*40.**3)
#endif
...
#  elif defined PZEFTFE
     &          , iNO3_, iPhy1, iZoo1
     &          , iDET1, iFer1, iDETFe1
     &          , iPmu1, iVar1, oPPt, omuNet, odmuNetdl,od2mudl2
     &          , oQN,   otheta,od2muNet_QNdl2,od2theta_QNdl2
     &          , oFe_lim, ofN, oSI,oPAR
#    ifdef CITRATE3
     &          , iMTo1, iMIo1, iVTo1, iVIo1
     &          , odmudx, od2mudx2, odmudZ, od2mudZ2
#    else
     &          , od3mudl3, od4mudl4 
#    endif
     &          , NumFluxTerms, NumGasExcTerms
     &          , NFlux_VSinkDET1, NFlux_VSinkDET2
     &          , NumVSinkTerms,NumFluxTermsN 

#  elif defined BIO_NPZDDISC
     &          , iNO3_, iPHY1(NPHY), iZOO1
     &          , iDET1
     &          , omuNet(NPHY),oSI(NPHY),oLno3(NPHY)
     &          , oPPt, oGraz(NPHY)
     &          , NumFluxTerms
     &          , NumFluxTermsN
     &          , NumGasExcTerms
...
#   endif
     &          , NFlux_Cpp
     &          , NFlux_Grazing, NFlux_Pmort, NFlux_Zmort
     &          , NFlux_Zeges, Nflux_Zremin,NFlux_ReminD
#   ifdef EFTSIZE
     &          , NFlux_dPMUdt,NFlux_dVARdt,NFlux_dmudl
     &          , NFlux_dgdl,  NFlux_dwdl,  NFlux_d2mudl2
     &          , NFlux_d2gdl2,Nflux_d2wdl2
#   endif
     &          , NumFluxTermsN
#   ifdef OXYGEN
     &          , OGain_NewProd, OLoss_Zmetab
...
#   endif
     ...
     &           NumVSinkTerms  = 2)
#  elif defined BIO_NPZDDISC
      parameter (ntrc_bio=3+NPHY, itrc_bio=itemp+ntrc_salt+ntrc_pas+1)
      parameter (iNO3_=itrc_bio,  ! Other parameters defined in init_scalars.F   
     &           NumFluxTerms   = 81,
     &           NumFluxTermsN  = 81,
     &           NFlux_VSinkDET1= 1,
     &           NumGasExcTerms = 0,
...
#   ifdef OXYGEN
      parameter (ntrc_bio=5,itrc_bio=itemp+ntrc_salt+ntrc_pas+1)
#   else
      parameter (ntrc_bio=4,itrc_bio=itemp+ntrc_salt+ntrc_pas+1)
#   endif
      parameter (iNO3_=itrc_bio,   
     &           iPhy1=iNO3_+1,
     &           iZoo1=iNO3_+2, 
...
#  if defined BIO_NChlPZD || defined BIO_N2ChlPZD2 || defined PISCES \
 ||   defined BIO_PZEFT   || defined BIO_BioEBUS   || defined BIO_FLEX\
 ||   defined PZEFTFE
